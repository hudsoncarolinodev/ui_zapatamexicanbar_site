/* 
    ...
    2) jQuery Cookie
    3) mathmedia
    4) matchmedia.addlistener
    5) Bootstrap
    6) BKNutritionCalc
    7) jQuery Smart Banner
    8) Owl Carousel 2.0.0
*/
...
/*
 * BK Nutrition Calculator
 */
function BKNutritionCalc() {
    this.memory = $.extend({}, this.default);

    return this;
};

BKNutritionCalc.prototype = {
    default: { "calories": 0,"transfat": 0,"sugar": 0,"saturatedfat": 0,"carbohydrates": 0,"sodium": 0,"protein": 0,"cholesterol": 0,"fat": 0 },
    memory: null,
    clear: function( key ) {
        return key ? this.memory[key] = 0 : this.memory = $.extend({}, this.default);
    },
    add: function( key, n ) {
        return this.memory[key] ? this.memory[key] += n : this.memory[key] = n;
    },
    subtract: function( key, n ) {
        return this.memory[key] ? this.memory[key] -= n : this.memory[key] = (0 - n);
    },
    ...
    getTotals: function( key ) {
        return key ? this.memory[key] : this.memory;
    },
    getRoundedTotals: function( key ) {
        if( key ) {
            return this.round(key, this.getTotals(key));
        } else {
            var totals = {};
            for(var key in this.memory) {
                totals[key] = this.round(key, this.memory[key]);
                //alert(totals[key]);
            }
        }
    ...
    round: function( key, value ) {
        function to( num, roundto ) {
            return roundto ? Math.round(num / roundto) * roundto : Math.round(num);
        }
        ...
        var method = {
            ...
            cholesterol: function( n ) {
                if( n < 2 ) {
                    return 0;
                } else if( n >= 2 && n <= 5 ) {
                    return Drupal.t('less than 5');
                } else {
                    return to(n,5);
                }
            },
        ...
        if(key === "transfat" || key === "saturatedfat") {
            return method.fat( value );
        } else if(key === "carbohydrates") {
            return method.sugar( value );
        } else {
...
!function ($) {
    var SmartBanner = function (options) {
        ...
        if (this.options.force) {
            ...
            if (UA.match(/Safari/i) != null || UA.match(/CriOS/i) != null) 
				this.type = 'ios' 
    ...
    SmartBanner.prototype = {
      ...
      , create: function() {
            ...
            if(this.options.url)
              ...
              if(this.type=='android') {
                ...
                // Override for DirectURL
                if(this.options.googlePlayDirectURL)
                    link = this.options.googlePlayDirectURL;
              }
              else
			  {
				...
				// Override for DirectURL
				if(this.options.appStoreDirectURL)
					link = this.options.appStoreDirectURL
			  }
				
            }

            var banner = '<div id="smartbanner" class="'+this.type+'"><div class="sb-container"><a href="#" class="sb-close">&times;</a><span class="sb-icon"></span><div class="sb-info"><strong>'+this.title+'</strong><span>'+this.author+'</span><span>'+inStore+'</span></div><a target="_blank" href="'+link+'" class="sb-button"><span>'+this.options.button+'</span></a></div></div>';
      ...
      , show: function(callback) {
            ...
            if (this.options.layer) {
                banner.animate({top: 0, display: 'block'}).addClass('shown').show();
                $('html').animate({marginTop: this.origHtmlMargin + (this.bannerHeight * this.scale)}, callback);
            } else {
      ...
      , hide: function(callback) {
            ...
            if (this.options.layer) {
                banner.animate({top: -1 * this.bannerHeight * this.scale, display: 'block'} ).removeClass('shown');
                $('html').animate({marginTop: this.origHtmlMargin}, callback);
            } else {
                if ($.support.transition) {
    ...
    $.smartbanner.defaults = {
        ...
        appStoreDirectURL: null,
        googlePlayDirectURL: null
    }
...
/*
 *  jQuery OwlCarousel v2.0.0-beta.2.4
 *
 ...
 *
 * Original file is at: date.format.js
 */
